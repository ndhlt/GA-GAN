...
class LoggingManager:
    def __init__(self, trainer_config):
        ...
        self.exp_logger = WandbLogger(
            logging=self.config.exp.logging,
        ...
        self.console_logger = ConsoleLogger(
            trainer_config.exp.name,
            trainer_config.exp.logging
        )
    ...
    def _init_local_dir(self):
        ...
        # switch local dir relativeness to cwd() relative
        if 'models_root' not in self.config.exp:
            root = Path(__file__).resolve().parent.parent.parent
        else:
            root = Path(os.getcwd()).resolve()
        
        exp_path = get_valid_exp_dir_name(
            root, 
            self.config.exp.name,
            models_root = self.config.exp.get('models_root', 'local_logged_exps')
        )
        if self.config.exp.logging:
...
class ConsoleLogger:
    def __init__(self, name, activate_logging):
        ...
        self.logger.disabled = not activate_logging
