...
class Net(nn.Module):
    ...
    def load_weights(self):
        ...
        if "latent_avg" not in checkpoint:
            self.latent_avg = self.generator.mean_latent(1024 * 4).squeeze()
        else:
    ...
    def cal_p_norm_loss(self, latent_in):
        # print(latent_in[0].shape)
        # print(self.X_mean.shape)
        # print(self.X_comp.shape)
        # print(self.X_stdev.shape)
        latent_p_norm = (
            torch.nn.LeakyReLU(5)(latent_in) - self.X_mean
...
class II2S(nn.Module):
    ...
    def invert_image(
        ...
        for step in pbar:
            optimizer.zero_grad()
            latent_in = torch.stack(latent, dim=0).unsqueeze(0)
            gen_im, _ = self.net.generator(
                [latent_in], input_is_latent=True, return_latents=False
            )
            im_dict = {
