...
def format_device(device_to_validate):
...
def get_valid_exp_dir_name(base_root, exp_name, models_root="local_logged_exps"):
    base_experiments_root = base_root / models_root
...
def get_trainable_model_state(config, state_dict):
    if config.training.patch_key == "original":
        ...
        ckpt = {
            ...
            "patch_key": config.training.patch_key,
            "state_dict": state_dict,
            "adaptive": config.training.get('adaptive', False)
        }
    ckpt['sg2_params'] = dict(config.generator_args['stylegan2'])
    return ckpt


w_idx_to_style_idx = dict([
    (0, 0),
    (1, 1),
    (1, 2),
    (2, 3),
