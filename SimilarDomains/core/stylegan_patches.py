...
class StyleDelta(Patch):
    ...
    def forward(self, style, offsets, alpha=1.):
        return style + alpha * offsets['in']
...
class WMod(Patch):
    ...
    def forward(self, style, offsets):
        mult = self.ones + offsets['w_offsets']
        return style * mult
...
@style_patches.add_to_registry('w_delta')
class WMod(Patch):
    def __init__(self, conv_weight: torch.Tensor):
        super().__init__()
        self.shape = conv_weight.shape
    ...
    def forward(self, style, offsets):
        return style + offsets['w_offsets']
...
class BaseModulationPatch(Patch):
    ...
    def forward(self, weight, offsets, alpha=1.):        
        return self._forward(weight, self._offset_mod(offsets, alpha))
    
    def _forward(self, weight, offsets):
        raise NotImplementedError()
    
    def _offset_mod(self, offsets, alpha):
        return offsets
...
class ChannelINMult(BaseModulationPatch):
    ...
    def _offset_mod(self, offsets, alpha):
        return {k: v * alpha for k, v in offsets.items()}
