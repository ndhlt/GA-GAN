...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, input, style, is_s_code=False, offset_power=1., offsets=None):
...
class ToRGB(nn.Module):
    ...
    def forward(self, input, style, skip=None, is_s_code=False):
        out = self.conv(input=input, style=style, is_s_code=is_s_code)
        out = out + self.bias
...
class DiscriminatorJojo(nn.Module):
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1]):
        super().__init__()
