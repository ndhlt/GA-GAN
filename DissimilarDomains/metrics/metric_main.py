...
def kid5k(opts):
    opts.dataset_kwargs.update(max_size=None)
    kid = kernel_inception_distance.compute_kid(
        opts, max_real=5000, num_gen=5000, num_subsets=100, max_subset_size=1000
    )
...
def fidxk(opts):
    ...
    fid = frechet_inception_distance.compute_fid(
        opts, max_real=opts.metric_kwargs.max_real, num_gen=opts.metric_kwargs.num_gen
    )
    return dict(fidxk=fid, max_real=opts.metric_kwargs.max_real, num_gen=opts.metric_kwargs.num_gen)
