...
class MetricOptions:
    def __init__(
        self, G=None, G_kwargs={}, dataset_kwargs={}, num_gpus=1, rank=0, 
        device=None, progress=None, cache=True, **metric_kwargs
    ):
        ...
        self.cache = cache
        self.metric_kwargs = dnnlib.EasyDict(metric_kwargs)
...
class ProgressMonitor:
    def __init__(
        self, tag=None, num_items=None, flush_interval=1000, 
        verbose=False, progress_fn=None, pfn_lo=0, pfn_hi=1000, pfn_total=1000
    ):
        self.tag = tag
...
def compute_feature_stats_for_dataset(
    ...
    data_loader_kwargs=None, max_items=None, num_workers=3, verbose=False, **stats_kwargs
):
    ...
    if opts.cache:
        ...
        args = dict(
            dataset_kwargs=opts.dataset_kwargs, detector_url=detector_url,
    ...
    for images, _labels in tqdm.tqdm(loader, total=len(loader), disable=not verbose):
        if images.shape[1] == 1:
