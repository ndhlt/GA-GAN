...
def subprocess_fn(rank, args, temp_dir):
    ...
    if args.num_gpus > 1:
        ...
        if os.name == 'nt':
            ...
            torch.distributed.init_process_group(
                ...
                rank=rank, world_size=args.num_gpus
            )
        else:
    ...
    for metric in args.metrics:
        ...
        result_dict = metric_main.calc_metric(
            metric=metric, G=G, dataset_kwargs=args.dataset_kwargs,
...
@click.option(
    '--metrics', help='Comma-separated list or "none"', type=CommaSeparatedList(),
    default='fid50k_full', show_default=True
)
@click.option(
    '--data', help='Dataset to evaluate metrics against (directory or zip) [default: same as training data]',
    metavar='PATH'
)
@click.option(
    '--mirror', help='Whether the dataset was augmented with x-flips during training [default: look up]',
    type=bool, metavar='BOOL'
)
@click.option('--gpus', help='Number of GPUs to use', type=int, default=1, metavar='INT', show_default=True)
@click.option(
    '--verbose', help='Print optional information', type=bool,
    default=True, metavar='BOOL', show_default=True
)
@click.option('--gpu', help='GPU idx to train with [default: None]', type=str)
def calc_metrics(ctx, network_pkl, metrics, data, mirror, gpus, verbose, gpu):
    """Calculate quality metrics for previous training run or pretrained network pickle.
