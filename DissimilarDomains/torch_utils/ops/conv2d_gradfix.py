...
def conv2d(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    if _should_use_custom_op(input):
        return _conv2d_gradfix(
            transpose=False, weight_shape=weight.shape, stride=stride, 
            padding=padding, output_padding=0, dilation=dilation, groups=groups
...
def conv_transpose2d(input, weight, bias=None, stride=1, padding=0, output_padding=0, groups=1, dilation=1):
    if _should_use_custom_op(input):
        return _conv2d_gradfix(
            transpose=True, weight_shape=weight.shape, stride=stride,
...
_warned_only_ones = False


def _should_use_custom_op(input):
    global _warned_only_ones
    ...
    if any(torch.__version__.startswith(x) for x in ['1.7.', '1.8.', '1.9']):
        return True
    if not _warned_only_ones:
        _warned_only_ones = True
        warnings.warn(
        ...
        def backward(ctx, grad_output):
            ...
            if ctx.needs_input_grad[0]:
                ...
                grad_input = _conv2d_gradfix(
                    transpose=(not transpose), weight_shape=weight_shape, 
