...
wavelets = {
    ...
    'db6': [
        -0.00107730108499558, 0.004777257511010651, 0.0005538422009938016, -0.031582039318031156,
    ...
    'sym7': [
        0.002681814568257878, -0.0010473848886829163, -0.01263630340325193, 0.03051551316596357,
        0.0678926935013727, -0.049552834937127255, 0.017441255086855827, 0.5361019170917628, 0.767764317003164,
        0.2886296317515146, -0.14004724044296152, -0.10780823770381774, 0.004010244871533663, 0.010268176708511255
    ],
    'sym8': [
...
def matrix(*rows, device=None):
    ...
    elems = [
        ...
        misc.constant(x, shape=ref[0].shape, device=ref[0].device) 
        for x in elems
    ]
...
class AugmentPipe(torch.nn.Module):
    def __init__(
        ...
        hue_max=1, saturation_std=1, imgfilter=0, imgfilter_bands=[1, 1, 1, 1], imgfilter_std=1,
    ...
    def forward(self, images, debug_percentile=None):
        ...
        if G_inv is not I_3:
            ...
            ) @ G_inv @ scale2d_inv(2 / shape[3], 2 / shape[2], device=device)
        ...
        if self.lumaflip > 0:
            ...
            i = torch.where(
                torch.rand([batch_size, 1, 1], device=device) < self.lumaflip * self.p, i,
                torch.zeros_like(i)
            )
            if debug_percentile is not None:
                i = torch.full_like(i, torch.floor(debug_percentile * 2))
        ...
        if self.hue > 0 and num_channels > 1:
            ...
            theta = torch.where(
                torch.rand([batch_size], device=device) < self.hue * self.p, theta,
        ...
        if self.saturation > 0 and num_channels > 1:
            ...
            s = torch.where(
                torch.rand([batch_size, 1, 1], device=device) < self.saturation * self.p, s,
                torch.ones_like(s)
            )
            if debug_percentile is not None:
        ...
        if self.imgfilter > 0:
            ...
            for i, band_strength in enumerate(self.imgfilter_bands):
                ...
                if debug_percentile is not None:
                    t_i = torch.full_like(
                        t_i, torch.exp2(torch.erfinv(debug_percentile * 2 - 1) * self.imgfilter_std)
        ...
        if self.noise > 0:
            ...
            sigma = torch.where(
                torch.rand([batch_size, 1, 1, 1], device=device) < self.noise * self.p, sigma,
